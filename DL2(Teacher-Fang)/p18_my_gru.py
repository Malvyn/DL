...
class Config:
    def __init__(self):
        ...
        self.lr = 0.0001
        self.epoches = 200
        self.save_path = 'models/p18_my_gru/my_gru'
        self.hidden_size = 4
    ...
    def __init__(self, config: Config):
        ...
        with tf.device('/gpu:0'):
            with tf.variable_scope('stock'):
                ...
                cell = MyGRUCell(config.stocks, config.hidden_size)
...
class MyGRUCell:
    ...
    def __call__(self, state, xi, name):
        with tf.variable_scope(name):
            merge = tf.concat((state, xi), axis=1)
            update_gate = self.gate(merge, 'update')
            input_gate = self.gate(merge, 'input')

            x_state = tf.concat((state * input_gate, xi), axis=1)
            x_state = tf.layers.dense(x_state, self.hidden_size, use_bias=False, name='dense1')
            x_state = tf.nn.tanh(x_state) * update_gate

            state = state * (1 - update_gate) + x_state
