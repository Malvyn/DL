...
class Tensors:
    ...
    def resnet(self, x, layers, persons):
        ...
        x = tf.layers.batch_normalization(x, axis=[1, 2, 3])
        x = tf.nn.relu(x)
    ...
    def resnet_module(self, x, resize, filters):
        ...
        left = tf.layers.batch_normalization(left, axis=[1, 2, 3])
        ...
        if resize or x.shape[-1].value != filters:
            ...
            right = tf.layers.batch_normalization(right, axis=[1, 2, 3])
...
class Face:
    ...
    def train(self):
        config = self.config
        steps_per_epoch = int(self.samples.num / config.batch_size / config.epoches)
