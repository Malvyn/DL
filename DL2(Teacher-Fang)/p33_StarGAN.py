...
class Config:
    def __init__(self):
        ...
        self.lr = 0.0002
        self.epoches = 1000
        self.save_path = 'models/stargan/stargan'
        self.eps = 1e-5
        self.gpus = get_gpus()
        self.logdir = '../log/stargan/'
        self.samples_path = '../deeplearning_ai12/p07_mnist/MNIST_data'
        self.semantics_size = 32
        self.image_path = 'images/p33_stargan.jpg'
...
class Tensors:
    def __init__(self, config: Config):
        ...
        with tf.variable_scope('stargan'):
        ...
        with tf.device('/gpu:0'):
            with tf.variable_scope('stargan_train'):
...
class SubTensors:
    def __init__(self, config: Config, gpu_index: int, opt):
        ...
        with tf.variable_scope('discriminator'):
            sample_p = discriminator(sample, sample_label)
            tf.get_variable_scope().reuse_variables()
            fake_p = discriminator(fake, target_label)
        ...
        with tf.variable_scope('loss_gradient'):
            ...
            self.loss2 = -tf.reduce_mean(tf.log(neg_fake_p))
