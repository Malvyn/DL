...
class Config:
    def __init__(self):
        ...
        self.lr = 0.0001
        self.epoches = 200
        self.save_path = 'models/p19_my_multi_lstm/my_multi_lstm'
        self.num_units = 4
        ...
        with tf.device('/gpu:0'):
            with tf.variable_scope('stock'):
                ...
                for i in range(config.num_step):
                    y_predict, state = cell(x[i, :, :], state, name='lstm_cell')
...
class MyMultiCell:
    ...
    def __call__(self, xi, state, name):
        with tf.variable_scope(name):
            new_state = []
            for i, s, cell in zip(range(len(state)), state, self.cells):
                xi, s = cell(xi, s, name='cell%d' % i)
                new_state.append(s)
            return xi, new_state
