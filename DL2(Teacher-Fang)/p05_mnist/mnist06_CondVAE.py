...
class Tensors:
    def __init__(self):
        ...
        self.z = tf.layers.dense(x, SEMAMTICS_SIZE)

        self.y = tf.placeholder(tf.int32, [None], name='y')
        y = tf.one_hot(self.y, 10)

        p = tf.layers.dense(self.z, 1000, activation=tf.nn.relu)
        p = tf.layers.dense(p, 7*7*64, activation= tf.nn.relu)
        p = tf.reshape(p, [-1, 7, 7, 64])

        p= tf.layers.conv2d_transpose(p, 32, 3, 2, 'same', activation=tf.nn.relu) #[14,14,32]
        p0 = tf.layers.conv2d_transpose(p, 1, 3, 2, 'same')  # [14,14,32]

        loss = tf.reduce_mean(tf.square(p0 - x0))
        self.lr = tf
