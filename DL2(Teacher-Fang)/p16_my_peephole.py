...
class Config:
    def __init__(self):
        ...
        self.lr = 0.0001
        self.epoches = 200
        self.save_path = 'models/p16_my_peephole/my_peephole'
        self.hidden_size = 4
    ...
    def __init__(self, config: Config):
        ...
        with tf.device('/gpu:0'):
            with tf.variable_scope('stock'):
                ...
                cell = MyPeepholeCell(config.stocks, config.hidden_size)
...
class MyPeepholeCell:
    ...
    def __call__(self, state, xi, name):
        ...
        with tf.variable_scope(name):
            merge = tf.concat((state, output, xi), axis=1)
