...
class Config:
    def __init__(self):
        ...
        self.save_path = 'models/p10_face_1vs1/p10_face_1vs1'  # The first p97_face is sub_directory, the 2nd one is model name
...
class Tensors:
    def __init__(self, config):
        with tf.device('/gpu:0'):
            ...
            logit = self.resnet(self.x/255, layers=resnet, persons=config.persons)  # [-1, 2048]
            self.logit = tf.nn.l2_normalize(logit, axis=1)

            w = tf.get_variable('w', [config.persons, logit.shape[-1].value])  # [persons, 2048]
            w = tf.nn.l2_normalize(w, axis=1)
            logit = tf.reshape(self.logit, [-1, 1, 2048])
            logit *= w  # [-1, persons, 2048]
            logit = tf.reduce_sum(logit, axis=2)  # [-1, persons]
            logit *= config.scale
    ...
    def resnet(self, x, layers, persons):
        ...
        x = tf.layers.flatten(x)  # [1, 1, 2048]
...
class Face:
    ...
    def predict(self):
        xs1, _ = self.samples.next_batch(self.config.batch_size)
        xs2, _ = self.samples.next_batch(self.config.batch_size)
        feed_dict = {
            self.ts.x: xs1,
            self.ts.training: False
        }
        logit1 = self.session.run([self.ts.logit], feed_dict)
        feed_dict[self.ts.x] = xs2
        logit2 = self.session.run([self.ts.logit], feed_dict)

        dot_mul = np.sum(logit1 * logit2, axis=1)
        result = np.greater(dot_mul, self.config.threshold)
